...
const AD_CONFIG = {
    top: {
        title: '상단 배너 광고 슬롯 예약하기',
        description: '웹사이트 최상단에 노출되는 가장 주목도 높은 배너입니다.',
        price: 100000,
        imageGuide: '권장 사이즈: 2560×224px (최소 1920×140px)',
        previewHeight: 'h-[70px] md:h-[80px]',
    },
    bottom: {
        title: '하단 배너 광고 슬롯 예약하기',
        description: '웹사이트 하단에 노출되는 배너입니다.',
        price: 50000, // Example price, adjust if needed
        imageGuide: '권장 사이즈: 2560×224px (최소 1920×140px)',
        previewHeight: 'h-[70px] md:h-[80px]',
    },
    newsletter: {
        title: '뉴스레터 배너 광고 슬롯 예약하기',
        description: '매일 발송되는 뉴스레터 하단에 포함되는 배너입니다.',
        price: 150000, // Example price, adjust if needed
        imageGuide: '권장 사이즈: 600×150px',
        previewHeight: 'h-[100px]',
    }
};
...
export default function AdBookingPage() {
    ...
    const type = params.type as keyof typeof AD_CONFIG;
    ...
    useEffect(() => {
        if (!config) {
            router.push('/ad'); // Redirect if invalid type
            return;
        }

        async function fetchBookedDates() {
            // TODO: Filter by ad type if you want separate availability for each slot
            // Currently fetching ALL bookings. You might want to add 'ad_type' column to ad_bookings table
            // and filter here. For now, assuming shared availability or just showing all.
            // Ideally: .eq('ad_type', type)
        ...
        const channel = supabase
            ...
            .on(
                'postgres_changes',
                {
                    ...
                    schema: 'public',
                    table: 'ad_bookings',
                    filter: "status=in.(paid,approved)",
                },
                () => {
                    fetchBookedDates();
                }
    ...
    const uploadImage = async (): Promise<string | null> => {
        ...
        const filePath = `ads/${type}/${fileName}`; // Organize by type
    ...
    const totalPrice = daysCount * (config?.price || 0);
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold text-gray-900 mb-2 text-center">{config.title}</h1>
                <p className="text-center text-gray-500 mb-8">{config.description}</p>
                            ...
                            {previewUrl ? (
                                <div className="space-y-3 animate-fade-in">
                                    <div className="border border-blue-200 rounded-lg p-3 bg-white shadow-sm">
                                        ...
                                        <div className="bg-gray-100 rounded overflow-hidden mx-auto">
                                            <div className={`${config.previewHeight} overflow-hidden relative`}>
                                                <img alt="Preview" className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 min-w-full min-h-full object-cover" src={previewUrl} />
                                            </div>
                                        </div>
                                ...
                                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm mb-6">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex justify-between text-sm">
                                            <span className="text-gray-500">광고 종류</span>
                                            <span className="font-medium text-gray-900">{config.title.replace(' 광고 슬롯 예약하기', '')}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
